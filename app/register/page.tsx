...
export default function Register() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      await axios.post(
        `${process.env.NEXT_PUBLIC_API_URL || "https://your-backend.onrender.com"}/api/auth/register`,
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4 relative overflow-hidden">
      ...
      <motion.div 
        ...
        className="absolute top-[-10%] right-[-10%] w-[40%] h-[40%] rounded-full bg-accent/20 blur-[100px]"
      />
      <motion.div 
        animate={{ 
          scale: [1, 1.5, 1],
        ...
        className="absolute bottom-[-10%] left-[-10%] w-[40%] h-[40%] rounded-full bg-primary/20 blur-[100px]"
      />
      ...
      >
        <div className="glass-panel rounded-3xl p-8 relative z-10">
          <div className="flex flex-col items-center mb-8">
            ...
            <p className="text-gray-400 mt-2">Join Real Chat App today</p>
          </div>
          ...
          <form onSubmit={handleRegister} className="space-y-4">
            ...
            <button
              type="submit"
              className="w-full bg-gradient-to-r from-secondary to-accent hover:opacity-90 text-white font-semibold py-3 rounded-xl transition-all active:scale-[0.98] mt-6 neon-glow"
            >
              Sign Up
            </button>
          </form>
          ...
          <p className="text-center text-gray-400 mt-6 text-sm">
            ...
            <Link href="/login" className="text-secondary hover:text-accent transition-colors font-medium">
              Sign In
            </Link>
          </p>
        </div>
